...
import { parsearNominaCsv, parsearRemuneracionesCsv, parsearArchivoExcelOJson, normalizarRun, normalizarRbd } from '@/lib/csvParser';
            ...
            <div className="bg-white rounded-xl shadow border border-slate-200/60 overflow-hidden">
              ...
              <div className="overflow-x-auto text-xs">
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filteredEsts.map(e => {
                      const estAlts = alertas.filter(a => normalizarRbd(String(a.rbd)) === normalizarRbd(String(e.rbd)) && !a.resuelta);
                      return (
                        <tr key={e.rbd} className="hover:bg-slate-50">
      ...
      {activeTab === 'dotacion' && (
        <main className="max-w-7xl mx-auto p-4 md:p-8 flex-1 flex flex-col gap-6 w-full animate-fadeIn">
          <div className="bg-white rounded-xl shadow border border-slate-200/60 p-6 space-y-6">
            ...
            {selectedDotacionRbd ? (() => {
              const schoolConts = contratos.filter(c => normalizarRbd(String(c.rbd)) === normalizarRbd(String(selectedDotacionRbd)));
