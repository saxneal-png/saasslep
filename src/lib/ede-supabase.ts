...
/**
 * Exportar leccionario de actividades en formato EDE (usa admin client)
 */
export async function exportarActividadesEDE(
  rbd: number,
  anioEscolar: number,
  limit?: number,
  offset?: number
  ...
  const { data, error } = await query;
  if (error) throw new Error(`exportarActividadesEDE: ${error.message}`);
...
/**
 * Exportar resúmenes mensuales de asistencia en formato EDE (usa admin client)
 */
export async function exportarResumenAsistenciaMesEDE(
  rbd: number,
  anioEscolar: number,
  limit?: number,
  offset?: number
  ...
  let query = supabase
    .from('vw_ede_resumen_asistencia_mes')
    .select('*')
    .eq('rbd', rbd)
    .eq('anio_escolar', anioEscolar)
  ...
  const { data, error } = await query;
  if (error) throw new Error(`exportarResumenAsistenciaMesEDE: ${error.message}`);
