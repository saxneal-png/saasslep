...
exports.normalizarRun = normalizarRun;
exports.parseDecimalHours = parseDecimalHours;
exports.parsearNominaCsv = parsearNominaCsv;
exports.parsearArchivoExcelOJson = parsearArchivoExcelOJson;
exports.parsearRemuneracionesCsv = parsearRemuneracionesCsv;
// @ts-ignore
const papaparse_1 = require("papaparse");
const XLSX = require("xlsx");
const types_1 = require("./types");
// Normalization function for RUN
function normalizarRun(runRaw) {
    if (runRaw === undefined || runRaw === null)
        return '';
...
function parsearNominaCsv(csvContent, rbdContext, controlPrevioJson, forceEstamento) {
    let rows = [];
    // Check if JSON
    ...
    rows.forEach((row, index) => {
        ...
        const limpiarCaracteresCorruptos = (str) => {
            if (!str)
                return '';
        ...
        let calidad_juridica = 'A contrata';
        ...
        const raw_funcion_principal = limpiarCaracteresCorruptos((row.Funcion ||
            row.funcion ||
            row.FUNCION_PRINCIPAL ||
            row.funcion_principal ||
            row.FUNCION_UNO ||
            row.funcion_uno ||
            'Auxiliar de Servicios').trim());
        let estamento = 'Asistente de la Educación';
        if (forceEstamento) {
        ...
        const funcion_principal = estamento === 'Docente'
            ? (0, types_1.normalizarCargoDocente)(raw_funcion_principal)
        ...
        const checkHasValue = (val) => val !== undefined && val !== null && val !== '';
        ...
        let legislacion_laboral = estamento === 'Docente' ? 'Estatuto docente' : 'Asistentes de la educación';
        ...
        const cleanDiscardValue = (val) => {
            if (val === undefined || val === null)
                return undefined;
            const clean = String(val).trim();
            const lower = clean.toLowerCase();
            if (lower === 'sin dato / agregar' || lower === 'no tiene' || lower === '--' || lower === '-')
                return undefined;
            return clean;
        };
        // Add unique Funcionario with estamento
        const titulo = cleanDiscardValue(row.Titulo ||
            row.titulo ||
            row.TITULO ||
            row.DOC_TITULO ||
            row.doc_titulo ||
        ...
        const cleanSigeNumber = (val) => {
            if (val === undefined || val === null || String(val).trim() === '--' || String(val).trim() === '-')
                return 0;
            return parseDecimalHours(val);
        };
        const cleanSigeString = (val) => {
            if (val === undefined || val === null || String(val).trim() === '--' || String(val).trim() === '-')
                return '';
...
function parsearArchivoExcelOJson(fileBuffer, fileName, rbdContext, controlPrevioJson, forceEstamento, schoolNameToRbdMap = {}) {
    const funcionarios = [];
    const contratos = [];
    const financiamientos = [];
    const alertas = [];
    ...
    workbook.SheetNames.forEach(sheetName => {
        ...
        const headers = rawRows[headerRowIdx].map(h => String(h || '').trim().toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, ""));
        const getIndex = (kws, fallback) => {
        ...
        for (let i = startRow; i < rawRows.length; i++) {
            ...
            let genero = sexVal;
            if (sexVal === 'M' || sexVal.startsWith('MASC'))
                genero = 'Masculino';
            else if (sexVal === 'F' || sexVal.startsWith('FEM'))
                genero = 'Femenino';
            ...
            if (tramoClean.includes('acceso'))
                tramo = 'Acceso';
            else if (tramoClean.includes('inicial'))
                tramo = 'Inicial';
            else if (tramoClean.includes('temprano'))
                tramo = 'Temprano';
            else if (tramoClean.includes('avanzado'))
                tramo = 'Avanzado';
            else if (tramoClean.includes('experto i') || tramoClean.includes('experto 1'))
                tramo = 'Experto I';
            else if (tramoClean.includes('experto ii') || tramoClean.includes('experto 2'))
                tramo = 'Experto II';
            ...
            if (!comuna) {
                ...
                if (ccLower.includes('bulnes'))
                    comuna = 'Bulnes';
                else if (ccLower.includes('carmen'))
                    comuna = 'El Carmen';
                else if (ccLower.includes('pemuco'))
                    comuna = 'Pemuco';
                else if (ccLower.includes('yungay'))
                    comuna = 'Yungay';
                else if (ccLower.includes('quillon') || ccLower.includes('quillón'))
                    comuna = 'Quillón';
                else if (ccLower.includes('san ignacio'))
                    comuna = 'San Ignacio';
                else
            ...
            const getCompletenessScore = (est, cargo, tr) => {
                let score = 0;
                if (est && est.trim() !== '')
                    score++;
                if (cargo && cargo.trim() !== '' && cargo !== 'Docente de Aula' && cargo !== 'Asistente')
                    score++;
                if (tr && tr.trim() !== '' && tr !== 'Sin Tramo')
                    score++;
                return score;
            };
            ...
            if (!func) {
                ...
                if (currentScore > existingScore) {
                    ...
                    if (tramo && tramo !== 'Sin Tramo')
                        func.tramo = tramo;
                    if (nombreCompleto && nombreCompleto !== 'Funcionario Sin Nombre')
                        func.nombre = nombreCompleto;
                    if (fechaIngreso)
                        func.fecha_ingreso_establecimiento = fechaIngreso;
                }
                else {
                    if (!func.genero && genero)
                        func.genero = genero;
                    if (!func.cargo && cargoRaw)
                        func.cargo = cargoRaw;
                    if ((!func.tramo || func.tramo === 'Sin Tramo') && tramo !== 'Sin Tramo')
                        func.tramo = tramo;
                    if (!func.fecha_ingreso_establecimiento && fechaIngreso)
                        func.fecha_ingreso_establecimiento = fechaIngreso;
                }
            }
            ...
            if (!isRelevanceZero) {
                ...
                if (tipoContClean.includes('titular'))
                    calidad_juridica = 'Titular';
                else if (tipoContClean.includes('plazo fijo') || tipoContClean.includes('plazofijo'))
                    calidad_juridica = 'Plazo fijo';
                else if (tipoContClean.includes('indefinido'))
                    calidad_juridica = 'Indefinido';
                else if (tipoContClean.includes('reemplazo'))
                    calidad_juridica = 'Reemplazo';
                else if (tipoContClean.includes('habilitacion') || tipoContClean.includes('habilitación'))
                    calidad_juridica = 'Habilitación especial';
                ...
                const upsertFinanciamiento = (origen, hrs) => {
                    if (hrs <= 0)
                        return;
                ...
                if (hasDirectDotaciones) {
                    ...
                    let origen = 'Subvención Regular';
                    if (progKey.includes('sep'))
                        origen = 'SEP';
                    else if (progKey.includes('pie'))
                        origen = 'PIE';
                    else if (progKey.includes('reforzamiento'))
                        origen = 'Reforzamiento';
                    else if (progKey.includes('retencion') || progKey.includes('retención'))
                        origen = 'Pro-retención';
                    else if (progKey.includes('bicentenario'))
                        origen = 'Liceos Bicentenarios';
                    else if (progKey)
                        origen = 'Otro';
...
function parsearRemuneracionesCsv(csvContent) {
    let rows = [];
    const trimmed = csvContent.trim();
    if (trimmed.startsWith('{') || trimmed.startsWith('[')) {
        try {
    ...
    rows.forEach((row, idx) => {
        ...
        const grupo_estamento = (est.includes('P01') || est.toLowerCase().includes('admin'))
            ? 'P01_Administrativo'
            : 'P02_Educacion';
